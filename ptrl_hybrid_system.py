...
def add_lstm_features(df: pd.DataFrame, ticker: str = "Unknown") -> pd.DataFrame:
    ...
    - LSTM_Pred_20d: T+20 預測漲跌幅
    - LSTM_Conf_20d: T+20 信心度 (MC Dropout)
    """
    global _LSTM_MODELS
    ...
    # 準備模型物件
    models = {
        '1d': (_LSTM_MODELS['model_1d'], _LSTM_MODELS['scaler_feat_1d'], _LSTM_MODELS['scaler_tgt_1d'], _LSTM_MODELS['meta_1d']),
        '5d': (_LSTM_MODELS['model_5d'], _LSTM_MODELS['scaler_feat_5d'], _LSTM_MODELS['scaler_tgt_5d'], _LSTM_MODELS['meta_5d']),
        '20d': (_LSTM_MODELS['model_20d'], _LSTM_MODELS['scaler_feat_20d'], _LSTM_MODELS['scaler_tgt_20d'], _LSTM_MODELS['meta_20d'])
    }
    ...
    # 批次處理每個模型的預測
    for time_horizon, (model, s_feat, s_tgt, meta) in models.items():
        if model is None: continue

        lookback = meta.get('lookback', 30 if time_horizon == '5d' else (60 if time_horizon == '20d' else 10))
        
        pred_col = f'LSTM_Pred_{time_horizon}'
        conf_col = f'LSTM_Conf_{time_horizon}'
        
        preds = np.zeros(n)
        confs = np.full(n, 0.5)
        
        if n < lookback + 10:
            df[pred_col], df[conf_col] = preds, confs
            continue

        try:
            scaled_feat = s_feat.transform(features)
            batch = [scaled_feat[i - lookback:i] for i in range(lookback, n)]
            ...
            if batch:
                ...
                # MC Dropout 採樣 30 次
                mc_samples = 30
                mc_results = [s_tgt.inverse_transform(model(batch, training=True).numpy()).flatten() for _ in range(mc_samples)]
                mc_mean, mc_std = np.mean(mc_results, axis=0), np.std(mc_results, axis=0)
                
                for j, idx in enumerate(range(lookback, n)):
                    if close_prices[idx] > 0:
                        preds[idx] = (mc_mean[j] - close_prices[idx]) / close_prices[idx]
                        cv = mc_std[j] / mc_mean[j] if mc_mean[j] > 0 else 0.1
                        
                        # 信心度分數計算
                        if time_horizon == '1d': # T+1: 0.8% ~ 4.0%
                            score = 1.0 - (cv - 0.008) / (0.040 - 0.008)
                        elif time_horizon == '5d': # T+5: 0.1% ~ 1.0%
                            score = 1.0 - (cv - 0.001) / (0.010 - 0.001)
                        else: # T+20: 1.0% ~ 3.0% (T+20 不確定性高，但 5% 太寬鬆，修正為 3%)
                            score = 1.0 - (cv - 0.010) / (0.030 - 0.010)
                            
                        confs[idx] = np.clip(score, 0.0, 1.0)
        except Exception as e:
            pass
