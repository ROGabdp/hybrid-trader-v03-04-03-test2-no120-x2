...
"""
================================================================================
Daily Operations - V4.1 Intraday Fixed LSTM (Hybrid Optimized)
================================================================================
盤中執行腳本 - 使用固定 LSTM 模型 (與回測使用相同的模型)
...
4. 輸出結果到 intraday_runs_v4_fixed/{date}_{time}/
5. 不寫入 twii_data_from_2000_01_01.csv
...
INTRADAY_RUNS_PATH = os.path.join(PROJECT_PATH, 'intraday_runs_v4_fixed')  # Fixed LSTM 專屬資料夾
...
def load_lstm_info_from_backtest(csv_file: str = None) -> dict:
    ...
    Args:
        csv_file: 已選擇的 CSV 檔案名稱 (e.g., 'daily_action_strat2_20250102_20251212.csv')
                  如果提供，會根據 CSV 的起始日期尋找對應的 lstm_info
    ...
    target_json = None
    if csv_file:
        # 從 CSV 檔名解析起始日期: daily_action_strat2_YYYYMMDD_YYYYMMDD.csv
        date_match = re.search(r'strat2_(\d{8})_', csv_file)
        if date_match:
            start_date_str = date_match.group(1)  # e.g., '20250102'
            # LSTM 訓練截止日 = 回測起始日 - 1 天
            from datetime import datetime, timedelta
...
def load_fixed_lstm_for_intraday(lstm_info: dict) -> bool:
    """
    載入 Fixed LSTM 模型到 core_system
    
    Args:
        lstm_info: 從 load_lstm_info_from_backtest() 取得的資訊
...
# Step 2: 隔離式特徵工程
# =============================================================================
def feature_engineering_with_fixed_lstm(intraday_data: tuple, avg_volume: float) -> pd.DataFrame:
    """
    特徵工程 (使用已載入的 Fixed LSTM 模型)
    
    LSTM 模型已在 load_fixed_lstm_for_intraday() 中載入到 core_system._LSTM_MODELS
    """
    print("\n" + "=" * 60)
    print("🔧 Step 2: 特徵工程 (使用 Fixed LSTM)")
    print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='V4 盤中分析系統 (Fixed LSTM)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
使用範例:
  python daily_ops_v4_intraday_fixed_lstm.py                    # 使用最新回測結果
  python daily_ops_v4_intraday_fixed_lstm.py -i                 # 互動式選擇回測 CSV
  python daily_ops_v4_intraday_fixed_lstm.py --backtest-start 2025-01-02  # 指定回測起始日
        '''
    )
    parser.add_argument(
        '-i', '--interactive',
    ...
    print(f"🚀 V4 盤中分析系統 (Fixed LSTM) - {date_str} {time_str}")
    ...
    if not load_fixed_lstm_for_intraday(lstm_info):
